...
function get_ptype_create_form_popup() {
	include('ajax/get_ptype_create_form_popup.php');
	die();
}
...
function protype_create_submit_popup() {
	global $wpdb;
	...
	if( isset( $_POST['product_type'] )){
		...
		$user_id = $_POST['user_id'];		
			...
			$product_typedetail = array(
					'name' => $product_type,
					'createdby' => $user_id,

				);
			$wpdb->insert($product_type_table,$product_typedetail);
			$data['success'] = 1;
...
add_action( 'wp_ajax_protype_create_submit_popup', 'protype_create_submit_popup' );
add_action( 'wp_ajax_nopriv_protype_create_submit_popup', 'protype_create_submit_popup' );
...
function get_product_type($name='') {
	global $wpdb;
					...
					$query = "SELECT * FROM `wp_product_type` WHERE `active`=1";
					$ptype['result'] = $wpdb->get_results($query);
					if(count($ptype['result'])>0 ) {
							foreach ($ptype['result'] as $ptype_value) {
							echo "<option";
							 if($name==$ptype_value->name){
							 	echo " selected";
							 	}
							 	echo " >$ptype_value->name</option>";
							}
						}
}
...
function gst_group_cgst($id = 0) {
	...
	$query = "SELECT sale_details.cgst_percentage,sale_details.igst_percentage,sale_details.hsn_code, 
...
function get_last_insert_stock_count($lotid=0){
global $wpdb;
$sale_table       	= $wpdb->prefix. 'stock';
$query ="SELECT bags_count FROM $sale_table  WHERE `lot_id`=$lotid ORDER BY id DESC LIMIT 1";
$data= $wpdb->get_row($query);
return $data->bags_count;
