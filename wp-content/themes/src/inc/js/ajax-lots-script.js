...
jQuery('.popup-ptype').live('click', function() { 
create_popup('get_ptype_create_form_popup', 'Add New Product Type');
});
...
//Validation
/*form submit*/
jQuery('#add_ptype').live('submit', function(){
    var product_type = jQuery('#product_type').val();   
    var user_id = jQuery('#user_id').val();

    if(product_type != '') {

         protype_create_submit_popup('protype_create_submit_popup',product_type,user_id);
    } else {
         alert_popup('<span class="error_msg">Enter the mandatory fields!!</span>', 'Alert!');
    }
});
function protype_create_submit_popup(action = '', product_type = '',user_id='') {
    jQuery.ajax({
        type: "POST",
